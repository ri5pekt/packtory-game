class_name EquipmentDeliveryBox
extends Node3D

## Pickup-able dock crate for an arrived equipment order.

signal emptied(box: EquipmentDeliveryBox)
...
func setup(delivery: Dictionary, world_position: Vector3, yaw_deg: float) -> void:
	...
	placeable_type = String(delivery.get("placeable_type", ""))
	delivery_label = String(delivery.get("label", "Equipment"))
	position = world_position
	rotation_degrees.y = yaw_deg
	add_to_group("equipment_delivery_boxes")
	_build_mesh()
	_build_click_area()
	_build_label()
...
func pickup_into(worker: Worker) -> bool:
	if worker == null:
		return false
	if worker.add_placeable_box(order_id, placeable_type, item_id, delivery_label):
		emptied.emit(self)
		queue_free()
		return true
	return false
...
func _build_label() -> void:
	...
	_label.set_empty("%s box" % delivery_label)
